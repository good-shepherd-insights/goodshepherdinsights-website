...
export const blogPostUrl = (post: SanityBlogPost | string, locale?: string) =>
  getLocaleUrlCTM(typeof post === "string" ? post : post.slug.current, locale, "/blog");
...
export const blogImageUrl = (
  ...
  const image = sanityImageUrl(post.coverImage.asset).width(width).auto("format");
...
export const categoryCounts = (posts: SanityBlogPost[]) => {
  ...
  posts.forEach((post) =>
    (post.categories || []).forEach((name) => counts.set(name, (counts.get(name) || 0) + 1)),
  );
  return [...counts].map(([name, count]) => ({ name, count, slug: slugifyyy(name) }));
};
